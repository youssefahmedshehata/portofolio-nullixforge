...
const tiles = Object.keys(ROUTES).map(id => {
  ...
  path.push({ p: 27.5, c: endAC, r: endAR, rot: 0 }); // quiet settle A (no red)
  
  // Unstable A
  let unstableA1 = (Math.random() > 0.5 ? 1.4 : -1.4);
  path.push({ p: 31, c: endAC, r: endAR, rot: unstableA1 });
  path.push({ p: 35, c: endAC, r: endAR, rot: -unstableA1 });
  ...
  path.push({ p: 51.5, c: endBC, r: endBR, rot: 0 }); // quiet settle B
  
  // Unstable B
  let unstableB1 = (Math.random() > 0.5 ? 0.9 : -0.9);
  path.push({ p: 55, c: endBC, r: endBR, rot: unstableB1 });
  path.push({ p: 59, c: endBC, r: endBR, rot: -unstableB1 });
  ...
  path.push({ p: 77.5, c: endFC, r: endFR, rot: 0 }); // quiet settle final
  path.push({ p: 90, c: endFC, r: endFR, rot: 0 }); // final hold
  ...
  return { id, path, redAStart, redBStart, greenStart };
});

fs.writeFileSync('./src/components/new_tiles.json', JSON.stringify(tiles, null, 2));
