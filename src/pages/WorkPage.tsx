...
export function WorkPage() {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      ...
      <section className="py-24 px-6 md:px-12 bg-[#0a0a0a]">
        <div className="max-w-7xl mx-auto space-y-12">
          {ALL_SYSTEMS.map((system) => (
            <div key={system.id} className="border border-[#212327] bg-[#0a0a0a] rounded-xl p-8 md:p-12 lg:p-16 flex flex-col lg:flex-row gap-12 lg:gap-24 relative overflow-hidden">
              <div className="lg:w-1/3 flex flex-col gap-6 relative z-10">
                <div className="flex items-center gap-4">
                  <span className="font-mono text-[12px] text-[#7d8187]">{system.id}</span>
                  <span className="font-mono text-[12px] text-white px-3 py-1 border border-[#212327] bg-black/5 rounded-full">{system.type}</span>
                </div>
              ...
              <div className="lg:w-2/3 flex flex-col gap-8 relative z-10">
                <div>
                  <h4 className="font-mono text-[12px] text-[#7d8187] mb-3">Diagnostic</h4>
                  <p className="text-[16px] text-[#7d8187] font-normal leading-relaxed">{system.problem}</p>
                </div>
                <div>
                  <h4 className="font-mono text-[12px] text-[#7d8187] mb-3">Implementation</h4>
                  <p className="text-[16px] text-[#7d8187] font-normal leading-relaxed">{system.solution}</p>
                </div>
                <div className="pt-6 border-t border-[#212327] flex flex-wrap gap-6">
                  {system.metrics.map(m => (
                    <div key={m} className="flex flex-col gap-1">
                       <span className="font-mono text-[12px] text-[#7d8187]">Metric</span>
      ...
      <section className="py-24 px-6 md:px-12 border-t border-[#212327] bg-[#0a0a0a]">
         <div className="max-w-7xl mx-auto">
            <h3 className="text-[32px] tracking-[-0.03em] font-semibold text-white mb-8">Work Principles</h3>
            <ul className="text-[#7d8187] font-normal leading-relaxed space-y-4 list-disc list-inside ml-4 max-w-3xl">
